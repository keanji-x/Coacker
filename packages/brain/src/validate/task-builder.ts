 ...
 */
export function buildUnderstandTask(
  issue: IssueItem,
  enrichment?: string,
): Task {
  return {
 ...
 * 构造 PR Create 任务
 */
export function buildPrCreateTask(
  issue: IssueItem,
  origin: string,
  isDraft = false,
  sastReport?: string,
  ...
  const messageParts = [
    `Create a ${isDraft ? "draft " : ""}pull request with the test code you generated for issue #${issue.number}.`,
    "",
    `Repository: ${origin}`,
    ...
    "3. Commit and push",
    `4. Create the PR using: gh pr create --title '...' --body '...'${isDraft ? " --draft" : ""}`,
  ];

  if (isDraft && sastReport) {
    messageParts.push(
      "",
      "## SAST Gate Report (failed — PR marked as draft)",
      "```",
      sastReport.slice(0, 2000),
      "```",
      "",
      "Include this SAST report in the PR body so reviewers can see the findings.",
    );
  }
