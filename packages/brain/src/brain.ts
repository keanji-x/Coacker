...
export class Brain {
  ...
  constructor(options: BrainOptions) {
    ...
    mkdirSync(join(this.outputDir, "conversations"), { recursive: true });
  }
  ...
  async run(player: Player): Promise<AuditReport> {
    ...
    this.persistConversation(intentionTask, intentionResult);
    this.persistState();

    const intentionResponse = this.getFirstResponse(intentionResult);
    this.subtasks = this.parseIntentionTasks(intentionResponse).slice(
      0,
      this.maxSubTasks,
    );
    ...
    // intention + gap 共享 maxSubTasks 預算, 总执行 subtask 数不超过 maxSubTasks
    ...
    for (let gap = 0; gap <= this.maxGapRounds; gap++) {
      ...
      this.gapRound = gap;
      this.log.info(
        `── Phase 2: Execution (round ${gap + 1}, ${pendingTasks.length} tasks, budget ${this.maxSubTasks - this.reports.size}/${this.maxSubTasks}) ──`,
      );
      ...
      for (const st of pendingTasks) {
        ...
        // 每个 subtask 完成后持久化对话 + 报告 + 状态
        this.persistConversation(task, result);
        ...
        // issue 已由 AI 在 propose_issues 步骤中通过 gh CLI 直接创建
        // 不再需要手动解析 response + submitIssues
      }

      // Gap Analysis — skip if budget exhausted or last round
      ...
      if (gap < this.maxGapRounds) {
        ...
        this.log.info(
          `── Phase 2.5: Gap Analysis (round ${gap + 1}, remaining budget: ${remainingBudget}) ──`,
        );

        const gapTask = this.buildGapTask();
        const gapResult = await player.executeTask(gapTask);
        this._history.push(gapResult);
        this.persistConversation(gapTask, gapResult);
        this.persistState();

        const gapResponse = this.getFirstResponse(gapResult);
        const newTasks = this.parseGapResult(gapResponse).slice(
          0,
          remainingBudget,
        ); // 共享 maxSubTasks 預算
        ...
        this.log.info(
          `  ⟳ Found ${newTasks.length} gaps (capped to budget), spawning new tasks`,
        );
    ...
    this.persistConversation(consolTask, consolResult);
  ...
  private persistConversation(task: Task, result: TaskResult): void {
    try {
      const convData = {
      ...
      writeFileSync(
        join(this.outputDir, "conversations", filename),
        JSON.stringify(convData, null, 2),
        "utf-8",
      );
    } catch (err) {
      this.log.warn(`Failed to persist conversation ${task.id}: ${err}`);
    }
  }
  ...
  private buildSubTask(st: SubTask): Task {
    ...
    const steps = [
      ...
      },
      {
        ...
        role: "reviewer",
        message:
          "Now perform a code quality review on the implementation you just analyzed.",
      },
      {
        id: "attack",
        role: "attacker",
        message:
          "Now act as a Red Team attacker. Compare the original user intention with the implementation you analyzed and find deep business logic flaws.",
      },
    ];
    ...
    if (this.origin) {
      steps.push({
        id: "propose_issues",
        role: "issue_proposer",
        message: `Based on the review and attack findings above, create GitHub issues directly using \`gh issue create --repo ${this.origin}\`. Only create issues for Critical or High severity findings. Remember to add "--label coacker-audit" to each issue.`,
      });
    }
  ...
  /** 解析 AI 的 issue proposals 并通过 gh CLI 提交到 GitHub */
  private submitIssues(taskId: string, proposalsText: string): void {
    ...
    const proposals = extractJSON<IssueProposal[]>(proposalsText);
    if (!Array.isArray(proposals) || proposals.length === 0) {
      this.log.info(`  📝 [${taskId}] No issues to submit`);
      return;
    }

    const [owner, repo] = this.origin.split("/");
    if (!owner || !repo) {
      this.log.warn(
        `  ⚠️ Invalid origin format "${this.origin}", expected "owner/repo"`,
      );
      return;
    }

    let submitted = 0;
    for (const issue of proposals.slice(0, 5)) {
      if (!issue.title || !issue.body) continue;

      try {
        // 在 body 开头加上来源标识
        const body = [
          `> 🤖 Auto-generated by Coacker audit (task: \`${taskId}\`)`,
          "",
          issue.body,
        ].join("\n");
        ...
        submitted++;
        this.log.info(`  🎫 [${taskId}] Issue created: ${result}`);
      } catch (err) {
        this.log.warn(
          `  ⚠️ [${taskId}] Failed to create issue "${issue.title}": ${err}`,
        );
      }
    }

    this.log.info(
      `  📝 [${taskId}] ${submitted}/${proposals.length} issues submitted to ${this.origin}`,
    );
  }
